...
if($countArr<=0 || $countArr == null){
	...
	for ( $i=0; $i<$count; $i++){
		...
		if (!isset($TwitOut[$j])) break;
			$out .= "\n<a href=\"http://twitter.com/" . $follower["screen_name"] . "\" title=\"" . $follower["screen_name"] . ": " . htmlspecialchars($follower["description"]) . "\" target=\"".$target."\"><img src=\"" . $follower["profile_image_url"] . "\" border=\"0\" style=\"width: {$size}px; height: {$size}px;\" /></a>";	
	}
	echo $out;
	echo "</div>";
	}}
